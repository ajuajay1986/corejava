...
public class GalaxyGuide {
	private HashMap<Character, Integer> roman_symbol_table = new HashMap<Character, Integer>();
	...
	// insert roman symbol values in roman symbol table
	private void insertSymbolValues() {
		roman_symbol_table.put('I', 1);
		roman_symbol_table.put('V', 5);
		roman_symbol_table.put('X', 10);
		roman_symbol_table.put('L', 50);
		roman_symbol_table.put('C', 100);
		roman_symbol_table.put('D', 500);
		roman_symbol_table.put('M', 1000);
	}
	...
	private int parse(String symbols) {
		if (symbols == null)
			return 0;
		if (symbols.length() == 1) {
			return roman_symbol_table.get(symbols.charAt(0));
		}
		decimal = 0;
//		boolean flag = true;
		for (int i = 0; i < symbols.length(); i++) {
			if (i + 1 < symbols.length()) {
				if (roman_symbol_table.get(symbols.charAt(i)) >= roman_symbol_table.get(symbols.charAt(i + 1))) {
					decimal = decimal + roman_symbol_table.get(symbols.charAt(i));
				} else {
					// subtracting the decimal value of the roman numeral as it is smaller than it's
					// next one.
					decimal = decimal + (roman_symbol_table.get(symbols.charAt(i + 1)) - roman_symbol_table.get(symbols.charAt(i)));
					i++;
				}
			} else {
				decimal = decimal + roman_symbol_table.get(symbols.charAt(i));
	...
	void processMessage(String[] message, int words) {
		// if "glob is I" type of input message
		...
		// if "glob glob silver is 34 credits" type of input messages
		...
		// if "how many credits is glob prok silver ?" type of input message
		...
		// if "how much is pish tegj glob glob ?" type of input questions
	...
	public static void main(String[] args) {
		...
		String[] galactic_message = new String[12];
